...
import { getGoogleRedirectUri } from '@/lib/appUrl';
...
export async function GET(req: NextRequest) {
      ...
      <html>
        ...
        <body style="font-family: system-ui; background: #070a13; color: #f8fafc; padding: 40px; line-height: 1.6;">
          <div style="max-width: 600px; margin: 0 auto; background: #0e1424; padding: 32px; border-radius: 24px; border: 1px solid #1e293b;">
            ...
            <p style="font-size: 14px; color: #94a3b8;">Add these credentials to your Vercel Project Environment Variables or <code>.env</code> file.</p>
  ...
  try {
    ...
    const errorMessage = err instanceof Error ? err.message : 'Missing production application domain configuration.';
    const configErrorHtml = `
      <!DOCTYPE html>
      <html>
        <head><title>Google OAuth Domain Configuration Error</title></head>
        <body style="font-family: system-ui; background: #070a13; color: #f8fafc; padding: 40px; line-height: 1.6;">
          <div style="max-width: 600px; margin: 0 auto; background: #0e1424; padding: 32px; border-radius: 24px; border: 1px solid #1e293b;">
            <h2 style="color: #f43f5e; margin-top: 0;">⚠️ Production Domain Required for Google OAuth</h2>
            <p>${errorMessage}</p>
            <div style="background: #070a13; padding: 16px; border-radius: 12px; font-family: monospace; font-size: 13px; color: #38bdf8; border: 1px solid #1e293b;">
              NEXT_PUBLIC_APP_URL="https://your-vercel-domain.vercel.app"
            </div>
            <p style="font-size: 14px; color: #94a3b8; margin-top: 12px;">Ensure this redirect URI is also added to Authorized redirect URIs in Google Cloud Console.</p>
            <a href="/login" style="display: inline-block; padding: 10px 20px; background: #06b6d4; color: #0f172a; text-decoration: none; border-radius: 12px; font-weight: bold; margin-top: 16px;">Return to Login</a>
          </div>
        </body>
      </html>
    `;
    return new NextResponse(configErrorHtml, { status: 500, headers: { 'Content-Type': 'text/html' } });
  }
