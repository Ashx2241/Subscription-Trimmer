...
export default function Sidebar() {
  ...
  const secondaryNav = [
    { label: 'Data Disclaimer', icon: Lock },
    { label: 'API & Webhooks', icon: Layers },
    { label: 'Terms & Compliance', icon: FileText },
  ...
  return (
    <aside className="w-64 dark-sidebar hidden lg:flex flex-col justify-between p-5 border-r border-slate-800/80 sticky top-0 h-screen select-none z-30">
      <div className="space-y-6 overflow-y-auto no-scrollbar">
        ...
        {/* User Card */}
        <div className="p-3 rounded-xl bg-[#0e1424] border border-slate-800/80 space-y-2">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-cyan-500 to-blue-600 text-white font-bold flex items-center justify-center text-xs shadow-md">
              JD
            </div>
          ...
          {primaryNav.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href} className="relative block">
                ...
                <div
                  className={`flex items-center justify-between px-3 py-2.5 rounded-lg text-xs font-semibold transition-colors relative z-10 ${
      ...
      <div className="pt-4 border-t border-slate-800/80 space-y-3">
        <button className="w-full py-2.5 px-4 rounded-xl bg-gradient-to-r from-cyan-500 via-emerald-500 to-teal-500 hover:from-cyan-400 hover:to-teal-400 text-slate-950 font-extrabold text-xs shadow-lg shadow-cyan-500/20 transition-all flex items-center justify-center gap-2">
